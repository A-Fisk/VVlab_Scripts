...
<<<<<<< HEAD
       ...
       for channel=1:num_channels
           ...
           for Chunk=1:(length(Timestamps_list)-1)
               ...
               %set the channel to be recording just the current channel
               TTX.SetGlobalV('Channel', channel);
               
               %Readevents to cache all the events which might make faster?
               TTX.ReadEventsV(max_events_return,... max number of events, very high
                   events_to_read,... which events we want to read,
                   channel,... which channel - going through all 
                   0,... sort code- ignore
                   start_read_time,... where to read from
                   end_read_time,... where to read to
                   'ALL');%Options read all 
