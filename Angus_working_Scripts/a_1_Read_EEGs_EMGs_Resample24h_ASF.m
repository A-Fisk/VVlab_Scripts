...
epochl=4;
%select the files Path is the overall directory containing tank directory
...
for dd=15:18
    ...
    for mouse=1:numanim
        ...
        %bunch of variables that define the data being dragged from TDT max
        %ret = max no of events to be returned - determines day length?
        ...
        while events>0     % reads events in steps % what is the end condition?
            ...
            %length event123 - file type and mouse number to call from
            %earlier Channel = 0 returns all channels SortCode =0 -
            %disregards (T1 - T2) - count events in this number step All =
            %Options string
        ...
        for ee=1:1
            ...
            for chan = 1:numch
                ...
                %initialise variables sig is list which continuous
                ...
                for ii=1:length(Tstamps)
                    ...
                    %grab the events from the file for just this chunk set
                    %the start, end, and channels as previously defined
